...
/* ---------- 3. 타입 판정 ---------- */
function pickTasteType(m,profiles){
  let best=null,bd=Infinity;
  for(const p of profiles){ const c=p.centroid||{}; let s=0;
    for(const k of MKEYS){ const d=clampM(m[k])-clampM(c[k]); s+=d*d; }
    const d=Math.sqrt(s); if(d<bd){bd=d;best=p;} }
  return best;
}

/* ---------- 4. 일치도 % ---------- */
function matchPercent(quality,mood,score){
  const pm=score.m||{}, pq=score.q||{};
  let md=0; for(const k of MKEYS){ const d=Math.round(clampM(mood[k]))-Math.round(clampM(pm[k])); md+=d*d; }
  const moodScore=1-Math.sqrt(md)/20;
  const qAttrs=STATE.qAttrs||[];
  let sumW=0,sumWS=0,penalty=0;
  for(const a of qAttrs){ const imp=Math.round(clampQ(quality[a.id])),lvl=Math.round(clampQ(pq[a.id]!=null?pq[a.id]:5)); sumW+=imp; sumWS+=imp*lvl; if(lvl<=2)penalty+=0.03; }
...
function gaugeInfo(sel){
  ...
  const scored=compared.map(p=>({id:String(p.id), m:matchPercent(input.quality,input.mood,scoreOf(scoreMap,p.id))})).sort((a,b)=>b.m-a.m || (a.id<b.id?-1:a.id>b.id?1:0));
...
const QUMB={'수납력':'실용성','활용도':'실용성','소재':'퀄리티','내구성':'퀄리티','마감':'퀄리티','무게':'가벼운 편안함','휴대성':'가벼운 편안함','희소성':'특별함'};
const MOODP={minimal:v=>v<0?'군더더기 없이 깔끔한':'포인트가 있는',color:v=>v<0?'차분한 톤의':'색감이 살아있는',cute_chic:v=>v<0?'사랑스러운':'시크한',daily_special:v=>v<0?'데일리하게 편안한':'특별한 자리에 어울리는'};
function buildSummary(quality,mood){
  const qAttrs=STATE.qAttrs||[];
  const umb={};
  qAttrs.forEach(a=>{ const v=clampQ(quality[a.id]); const e=Math.max(0,v-5)/5; const u=QUMB[a.name]||a.name; if(!(u in umb)||e>umb[u]) umb[u]=e; });
  const qTop=Object.entries(umb).filter(([,e])=>e>0.001).sort((x,y)=>y[1]-x[1]).map(([u])=>u).slice(0,2);
  const mTop=MKEYS.map(k=>({k,v:clampM(mood[k]),e:Math.abs(clampM(mood[k]))/5})).filter(x=>x.e>0.001).sort((a,b)=>b.e-a.e).slice(0,2).map(x=>MOODP[x.k](x.v));
  const seg=[];
  if(qTop.length){ const w=qTop.join('과 '); seg.push(`${w}${objParticle(qTop[qTop.length-1])} 중요하게 보고`); }
  if(mTop.length){ seg.push(`${mTop.join(' ')} 무드를 좋아하는 편이에요`); }
  if(!seg.length) return '특정 항목에 치우치지 않고 전반적으로 균형 있게 보는 편이에요.';
  if(seg.length===1) return seg[0].replace(/ 보고$/,' 보는 편이에요')+'.';
  return seg.join(', ')+'.';
}
...
function topCatSlug(slug,cats){ let c=(cats||[]).find(x=>x.slug===slug); let g=0; while(c&&c.parent_id&&g++<6){ const p=(cats||[]).find(x=>x.id===c.parent_id); if(!p)break; c=p; } return c?c.slug:slug; }
...
async function loadData(input){
  ...
  const { data:attrDefs }=await sb.from('attribute_definitions').select('*').eq('is_active',true).order('sort_order');
  const { data:cats }=await sb.from('categories').select('id,slug,parent_id');
  const allIds=[...new Set([...(allProducts||[]).map(p=>p.id), ...compared.map(p=>p.id)])];
  const paRes = allIds.length ? await sb.from('product_attributes').select('product_id,attribute_id,value').in('product_id',allIds) : {data:[]};
  const paRows = paRes.data || [];
  const moodCode={minimal_deco:'minimal',mono_color:'color',cute_chic:'cute_chic',daily_special:'daily_special'};
  const moodById={}; (attrDefs||[]).forEach(a=>{ if(a.attr_type==='style'&&moodCode[a.code]) moodById[a.id]=moodCode[a.code]; });
  const scoreMap={};
  paRows.forEach(r=>{ const pid=String(r.product_id); if(!scoreMap[pid]) scoreMap[pid]={q:{},m:{minimal:0,color:0,cute_chic:0,daily_special:0}};
    const v=parseFloat(r.value); if(moodById[r.attribute_id]!=null) scoreMap[pid].m[moodById[r.attribute_id]]=v; else scoreMap[pid].q[r.attribute_id]=v; });
  return { profiles:profiles||[], compared, allProducts:allProducts||[], scoreMap, attrDefs:attrDefs||[], cats:cats||[] };
}
const scoreOf=(map,id)=>map[String(id)]||{q:{},m:{minimal:0,color:0,cute_chic:0,daily_special:0}};

/* ---------- 8. 슬라이더 ---------- */
function renderSliders(input){
  const q=input.quality||{},m=input.mood||{};
  const qAttrs=STATE.qAttrs||[];
  const qRow=a=>`<div class="sld-row"><span class="sld-label">${a.name}</span><div class="sld-track"><div class="sld-dot" style="left:${clampQ(q[a.id])/10*100}%"></div></div><span class="sld-pole-r"></span></div>`;
  ...
  return `<div class="sld-group-title">품질 <small>높을수록 중요하게 생각해요</small></div><div class="sld-rows">${qAttrs.map(qRow).join('')}</div>
...
(async function init(){
  ...
  try{
    ...
    // 비교 카테고리(하위면 상위로 올림) → 품질 항목 확정 → STATE에 먼저 넣어야 매칭 계산 가능
    ...
    const ranked = data.compared
      ...
      .sort((a,b)=> b.m-a.m || (a.id<b.id?-1:a.id>b.id?1:0));
    if(ranked[0]) defaultSel = ranked[0].id;
    // 공유 보기일 땐 공유한 사람이 고른 상품을 기본 선택으로
    if(input.share && input.selected) defaultSel = String(input.selected);
    STATE.type = type; STATE.selectedId = defaultSel;
    render();
